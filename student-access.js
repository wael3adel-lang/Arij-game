(function(){
  ...
  overlay.innerHTML='<div class="accessBox"><div style="font-size:48px">🔒</div><h2>Bu oyuna erişim yok</h2><p>Bu oyun size atanmadı.</p><button onclick="location.href=\'../index.html\'">Oyunlarım</button></div>';
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    if(!code||!window.supabaseClient){overlay.style.display='flex';return;}
    try{
      const {data,error}=await window.supabaseClient.rpc('get_student_access',{p_code:code});
      if(error) throw error;
      const allowed=(data||[]).some(r=>String(r.game_id||'')===GAME_ID);
      if(!allowed) overlay.style.display='flex';
    }catch(e){console.error('ARİJ access check failed',e);overlay.style.display='flex';}
  });
})();
